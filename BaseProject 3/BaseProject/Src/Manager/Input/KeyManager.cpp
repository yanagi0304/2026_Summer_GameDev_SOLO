...
#pragma region プレイヤー操作
	...
	// メイントリガー（基本的に「通常攻撃」）
	SET_KEYBOARD(KEY_TYPE::PLAYER_MAIN, KEY_INPUT_J);
	SET_KEYBOARD(KEY_TYPE::PLAYER_MAIN, KEY_INPUT_RETURN);
	SET_MOUSE_BUTTON(KEY_TYPE::PLAYER_MAIN, MOUSE_INPUT_LEFT);
	SET_C_BUTTON(KEY_TYPE::PLAYER_MAIN, XINPUT_BUTTON_B);
	//SET_C_BUTTON(KEY_TYPE::PLAYER_MAIN, XINPUT_BUTTON_RIGHT_SHOULDER);
	...
	//メイントリガーの切り替え
	SET_KEYBOARD(KEY_TYPE::PLAYER_MAIN_SWITCH,KEY_INPUT_DOWN);
